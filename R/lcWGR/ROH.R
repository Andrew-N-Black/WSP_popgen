...
ggplot(melt_data, aes(fill=variable, y=value, x=reorder(ID,value))) +geom_bar(stat="identity",position="dodge")+theme(axis.title.x=element_blank(),axis.text.x=element_blank(),axis.ticks.x=element_blank())+theme_classic()+facet_grid(variable~Pop,scales = "free")+
    scale_fill_manual(values=c("#9933FF","darkred"))+ylab("fROH")+xlab("Sample")
